...
opt[ijo]ji.k
    ...
    feats = np.zeros((len(image_paths), 16 * 16))

    for i, image_data in enumerate(image_paths):
        image = Image.open(image_data)
        image_tiny = image.resize((16, 16), Image.BICUBIC)

        # image = np.asarray(load_image_gray(image_data))
        # image_tiny = image.resize((16, 16))
        # print(image_tiny)
        # print("SS")
        # print(np.asarray(Image.open(image_data).resize((16,16),Image.ANTIALIAS)))
        image_tiny = np.asarray(image_tiny).flatten()
        feats[i, :] = (image_tiny - np.mean(image_tiny)) / np.std(image_tiny)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for path in image_paths:
        ...
        frames, descriptors = vlfeat.sift.dsift(image, step=[step, step], fast=True)
...
def get_bags_of_sifts(image_paths, vocab_filename):
    ...
    for i, path in enumerate(image_paths):
        image = np.asarray(Image.open(path), dtype='float32')
        frames, descriptors = dsift(image, step=[step, step], fast=True)

        dist = distance.cdist(vocab, descriptors, metric='euclidean')
        H, B = np.histogram(np.argmin(dist, axis=0), range(len(vocab) + 1))
        if np.linalg.norm(H, keepdims=True) == 0:
            feats[i, :] = H
        else:
            feats[i, :] = H / np.linalg.norm(H, keepdims=True)

    # for path in image_paths:
    #     img = np.asarray(Image.open(path), dtype='float32')
    #     frames, descriptors = dsift(img, step=[9, 9], fast=True)
    #     dist = distance.cdist(vocab, descriptors, metric='euclidean')
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
    ...
    for i in range(len(dist[0])):
        ...
        neibers = dict()
        for j in range(10):
            if train_labels[ans[j]] in neibers.keys():
                neibers[train_labels[ans[j]]] += 1
            else:
                neibers[train_labels[ans[j]]] = 1

        sort_neibers = sorted(neibers.items(), key=operator.itemgetter(1), reverse=True)
        label_list = []
        for j in range(len(sort_neibers)):
            if sort_neibers[j][1] == sort_neibers[0][1]:
                label_list.append(sort_neibers[j][0])
            else:
                break
        next = label_list[random.randrange(len(label_list))]
        test_labels.append(next)
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    hyperplane = np.zeros(len(train_labels))
    ...
    for i in range(len(categories)):
        for j in range(len(train_labels)):
            hyperplane[j] = categories[i] == train_labels[j]

        judged_label = np.zeros(len(hyperplane))
        for k in range(len(judged_label)):
            if hyperplane[k]: judged_label[k] = 1

        svms[categories[i]].fit(train_image_feats, judged_label, sample_weight=1)
        W_list.append(svms[categories[i]].coef_)
        B_list.append(svms[categories[i]].intercept_)
    ...
    for i in range(test_image_feats.shape[0]):
        ...
        for j in range(len(categories)):
            judge = np.dot(W_list[j, 0, :], test_image_feats[i, :]) + B_list[j, :]
            judge_list.append(judge)
        for label_num in range(0, len(categories)):
            if judge_list[label_num] == max(judge_list):
                test_labels.append(categories[label_num])
